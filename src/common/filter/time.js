...
//将日期设置为当月的最后一天
export function addMouthDate(date) {
  let d = new Date(date.getFullYear(), date.getMonth() + 1, 1);
  d.setHours(d.getHours() - 1);
  return d;
}

export function compareLastDay(date) {
  let d = new Date(date.getFullYear(), date.getMonth() + 1, 1);
  d.setHours(d.getHours() - 1);

  if (d.getDate() - date.getDate() === 0) {
    return true;
  } else {
    return false;
  }
...
//将当前日期到当月月底加上下一个月的天数
export function addOneMouthDate(date) {
  let d1 = new Date(date.getFullYear(), date.getMonth() + 2, 1);
...
//两个时间相减得到的日期
export function reduceDate(date1, date2) {
  let d1 = new Date(date1);
  let d2 = new Date(date2);
  return (Math.floor((d1.getTime() - d2.getTime()) / 1000 / 3600 / 24 + 1));
}

//两个时间相减得到的日期
export function reduceRealDate(date1, date2) {
  let d1 = new Date(date1);
  let d2 = new Date(date2);
  if (Math.abs((d1.getTime() - d2.getTime()) / 1000 / 3600 / 24) >= 1) {
    return (Math.floor((d1.getTime() - d2.getTime()) / 1000 / 3600 / 24));
  } else {
    return 0;
  }
...
//两个时间相减得到的月份
export function reduceMouth(date1, date2, calc) {
  let d1 = new Date(date1);
  let d2 = new Date(date2);
  //是否用于计算
  if (calc) {
    if (d2.getDate() === 1) {
      return reduceOverYear(d1, d2);
    } else {
      return reduceOverYear(d1, d2) - 1;
    }
  } else {
    if (d2.getDate() === 1) {
      if (compareLastDay(d1)) {
        return reduceOverYear(d1, d2) + 1;
      } else {
        return reduceOverYear(d1, d2);
      }
...
export function reduceOverYear(d1, d2) {
  if (d1.getFullYear() - d2.getFullYear() > 1) {
    return (d1.getFullYear() - d2.getFullYear()) * 12 + (12 - d2.getMonth());
  } else if (d1.getFullYear() - d2.getFullYear() === 1) {
    return (12 - d2.getMonth()) + d1.getMonth();
  } else if (d1.getFullYear() - d2.getFullYear() === 0) {
    return (d1.getMonth() - d2.getMonth());
  }
}

//两个时间相减得到的月份
export function reduceFullMouth(date1, date2) {
  let d1 = new Date(date1);
  let d2 = new Date(date2);
  if (d2.getDate() === 1) {
...
//最后一个月的时间的日期
export function reduceFirstDate(date) {
  let d = new Date(date);
  if (d.getDate() === 1) {
    return 1;
  } else {
