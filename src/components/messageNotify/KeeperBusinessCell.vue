<template>
	<div>
		...
		<div class="mv5" v-else-if="
		 messageNotify.action === Action.KEEPER_BUSINESS_NEW
		 || messageNotify.action === Action.KEEPER_BUSINESS_PASS
		 || messageNotify.action === Action.KEEPER_BUSINESS_REJECT
		">
			<UserProfileLink :user="messageNotify.sender"/>
			将状态修改为
		...
		<div class="mv5" v-if="messageNotify.notifications && messageNotify.notifications.length && user.hasPermission(FeatureType.KEEPER_BUSINESS_MANAGE)">
