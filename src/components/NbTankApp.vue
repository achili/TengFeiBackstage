<template>
  <div class="nb-tank-app">
    ...
    <NbExpanding>
      <div class="tiny-block" style="width: 320px;"
           v-if="tank.exist()">
        <!--:style="'width:'+previewWidth+'px'"-->

        <div class="bg-white  border"
             v-show="preview && tank.publicImgUrl()">
          <img width="320" height="540" :src="tank.publicImgUrl()"/>
        </div>
        <span class="download">
            <i class="btn-action f36 fa fa-arrow-circle-down text-success" @click.stop.prevent="tank.download()"></i>
        </span >
        <span class="f36 black pull-left cursor hover-underline download" @click.stop.prevent="tank.download()"> </span>
        <span class="del" v-if="edit">
					  <i class="btn-action f36 fa fa-times-circle text-danger" @click.stop.prevent="del()"></i>
        </span>
        ...
        <div class="mt10" align="center">
          <span style="color: #00b5e2;font-size: 18px;font-weight: normal;font-stretch: normal;">
            {{tank.name}}
