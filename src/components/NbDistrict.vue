<template>
    ...
    <div class="row">
      <div class="col-sm-3 item mt5" v-for="(addressArr, index) in choiceDistrictArr" :key="addressArr.id">
        <select class="form-control m-b" v-model="choiceDistrictArr[index]" @change="onChange(choiceDistrictArr[index])">
          <option v-for="address in districtLevelArr[index]" :value="address" :key="address.uuid">
            {{address.name}}
          </option>
        </select>
      </div>
