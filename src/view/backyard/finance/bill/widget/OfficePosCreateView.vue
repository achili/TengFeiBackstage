  ...
  <div class="row">
    <div class="col-xs-12" v-if="isC">
      <div class="row mt10">
        ...
        <div class="col-md-10">
          <input type="text" class="form-control" size="12" maxlength="12" v-model="financeOfficePay.no" disabled>
        </div>
      </div>
      <div class="row mt10">
        <label class="col-md-2 control-label mt5 compulsory">支付金额：</label>
        <div class="col-md-10">
          <input type="text" class="form-control" size="12" maxlength="12" v-model="financeOfficePay.amountPayable" disabled>
        </div>
      </div>
      <div class="row mt10">
...
<script>
  ...
  export default{
    ...
    watch: {
      'financeBillReceipt.category'(newVal, oldVal){
        ...
        if(this.financeBillReceipt.category === FinanceBillReceipt.prototype.Category.RENTAL){
          ...
          //this.financeBillReceipt.amount = (this.financeBillReceipt.financeOfficePay.total - this.financeBillReceipt.financeOfficePay.performanceBond - this.financeBillReceipt.financeOfficePay.rentalPaid).toFixed(2);
        }
    ...
    methods: {
      ...
      confirm() {
        console.log(this.financeOfficePay.status)
        if(this.financeOfficePay.status !== "REFUND"){
          this.PosTest();
        }
      },
      ...
      ToggleConnectionClicked() {
        let that = this;
        let flag = 0;
        that.numerical = that.financeOfficePay.amountPayable;
        ...
        let DUmsocx1 = this.financePos.field_apptype + xx + this.financePos.field_memo;
          try {
            ...
            that.ws.onopen = function(event){
              ...
              //等待支付
              that.payMoney = that.financeOfficePay.amountPayable;
            ...
            that.ws.onmessage = function(event){
              ...
              if(event.data.slice(0,2) === "00"){  //支付成功
                ...
                if(that.financeOfficePay.amountPayable > 0){
                  that.officeReceivable(that.successCallback, that.errorCallback);
                }
                that.payMoney = that.financeOfficePay.amountPayable;
      ...
      officeReceivable(successCallback, errorCallback){
        let financeBillReceipt = new FinanceBillReceipt();
        ...
        financeBillReceipt.category = "ROOM";
        financeBillReceipt.type = that.financePos.pos_field_apptypes;
        financeBillReceipt.amount = this.financeOfficePay.amountPayable;
