  ...
  <div class="row">
    <div class="col-xs-12" v-if="isC">
      ...
      <div class="row mt10">
        ...
        <div class="col-md-10">
          <input type="text" class="form-control" size="12" maxlength="12" v-model="financeOfficePay.amountPaid" disabled>
        </div>
      </div>
...
<script>
  ...
  export default{
    ...
    watch: {
      ...
      'financeOfficePay.amountPaid'() {
        if(this.financeOfficePay.amountPaid > 0){
          console.log("根据退款金额的改变检测是否加载");
          this.refreshes();
        }
      }
    },
    methods: {
      ...
      ToggleConnectionClicked() {
        let that = this;
        let flag = 0;
        that.numerical = that.financeOfficePay.amountPaid;
        ...
        let DUmsocx1 = that.financePos.field_apptype + xx + that.financePos.field_org_date + that.financePos.field_org_ref + that.financePos.field_org_transno;
          try {
            ...
            that.ws.onopen = function(event){
              ...
              //等待支付
              that.payMoney = that.financeOfficePay.amountPaid;
            ...
            that.ws.onmessage = function(event){
              ...
              if(event.data.slice(0,2) === "00"){  //支付成功
                if(that.financeOfficePay.amountPaid > 0){
                  that.officePosRefund(that.successCallback, that.errorCallback);
                }
                that.payMoney = that.financeOfficePay.amountPaid;
      ...
      officePosRefund(successCallback, errorCallback){
        let financeBillReceipt = new FinanceBillReceipt();
        ...
        financeBillReceipt.amount = -this.financeOfficePay.amountPaid;
      ...
      refreshes(successCallback, errorCallback){
        ...
        if(uuid){
          that.financeBillReceipt.httpOfficePosRefundDetail(uuid, function (response) {
