    ...
    <div class="body">
      ...
      <div class="invite-share-header-content">
        <div class="invite-share-header-content-first-cell">
          ...
          <span class="invite-share-header-content-first-cell-name">奕桥</span>
        </div>
      ...
      <div style="padding-left: 20px;padding-right: 20px;">
        <div class="mt10">
          ...
          <div>
            <input type="text" class="form-control" v-model="inviteVisitorUser.name" style="-webkit-appearance: none;">
          </div>
        </div>
        <div class="mt10">
          <div class="mb10">您的手机号码</div>
          <div>
            <input type="text" class="form-control" v-model="inviteVisitorUser.phone" style="-webkit-appearance: none;">
          </div>
        </div>
        ...
        <!--生成邀请访问二维码-->
        <div id="qrCode" class="w200 h200 bg-white" v-show="qrCode" style="margin: 40px auto;text-align: center" @click="refreshQrCode">
          <div v-show="currentTime.getTime() < inviteVisitor.invitingTime.getTime()">
            您预约的时间还未到，参观时间前15分钟即可获得通行证
            <div><i class="fa fa-refresh" :class="inviteVisitorUser.loading? 'fa-spin fa-fw':''"></i></div>
          </div>
          <div v-show="entryTime.getTime() > inviteVisitor.accessTime.getTime()">
            该预约参观已过期
          </div>
        </div>
...
<script>
  ...
  export default {
    data () {
      ...
      return {
        inviteVisitorUser: new InviteVisitorUser(),//邀请访客用户
        inviteVisitor: new InviteVisitor(),//邀请访客
        phoneValidation: this.$store.state.user.phoneValidation,//短信验证码对象
        qrCode: false,//二维码生成器对象
        isSuccess: false,
        inviteVisitorUserIdKey: "the_bridge_ivuser_",
        // startTime: startTime,
        // endTime: new Date(new Date().getTime() + 105 * 60 * 1000),
        entryTime:new Date(),
        currentTime: new Date(new Date().getTime() + 15 * 60 * 1000),
        address: {
          lng: 120.376062,
          lat: 30.310416,
          text: "上杭州经济开发区科技园路2号9号楼1层"
        }
      }
    },
    methods: {
      ...
      fetchPhoneValidate() {
        this.phoneValidation.phone = this.inviteVisitorUser.phone;
        this.phoneValidation.httpFetchPhone();
      },
      createQrCode(){
        if(this.inviteVisitorUser.qrCode) {
          console.log(this.inviteVisitorUser.qrCode)
          if(!this.qrCoder) {
            this.qrCoder = new QRCode('qrCode', {
              width: 200,
              height: 200, // 高度
              text: this.inviteVisitorUser.qrCode// 二维码内容
            });
          } else {
            this.qrCoder.makeCode(this.inviteVisitorUser.qrCode);
          }
      ...
      save() {
        ...
        this.inviteVisitor.httpCreateUser(this.inviteVisitor.uuid,this.inviteVisitorUser.name,
          this.phoneValidation.phone,this.phoneValidation.value, function (response) {

            that.inviteVisitorUser.render(response.data.data);
            that.isSuccess = true;
            that.qrCode = true;
            that.createQrCode();
            saveToLocalStorage(that.inviteVisitorUserIdKey + that.inviteVisitor.uuid, that.inviteVisitorUser.uuid);
            Message.success({
              message: '认证成功~'
            });
          });
      ...
      refreshQrCode() {
        let that = this;
        this.inviteVisitorUser.uuid = readLocalStorage(this.inviteVisitorUserIdKey + this.inviteVisitor.uuid);
        if(this.inviteVisitorUser.uuid && this.inviteVisitorUser.uuid !== 'undefined') {
          this.inviteVisitorUser.httpDetail(function () {
            that.isSuccess = true;
            that.createQrCode();
            that.qrCode = true;
    ...
    mounted(){
      //测试的时候用于清除本地存储
      // removeLocalStorage(this.inviteVisitorUserIdKey + this.inviteVisitor.uuid);
  ...
  .invite-share-detail {
    .body{
      ...
      .invite-share-header-picture {
        background: url("../../assets/img/invite.png") 100% 0;
